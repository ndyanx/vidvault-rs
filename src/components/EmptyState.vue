...
async function loadThumbs(history) {
  const results = await Promise.all(
    history.map(async (entry) => {
      ...
      const url = await invoke('store_get_folder_thumb', { dirPath: entry.path })
...
onMounted(async () => {
  ...
  _unlistenEnter = await listen('tauri://drag-enter', () => { isDragging.value = true })
  _unlistenLeave = await listen('tauri://drag-leave', () => { isDragging.value = false })
})
